...
import { CalendarProps, DataToView, EventDispatcher, EventType } from '../shared-types';
...
import { defaultProps } from './use-props';
...
export default function useDispatcher (args: Args): EventDispatcher {
  const {
    bind: {
      order,
      shared
...
function dispatcherFactory (args: Args): EventDispatcher {
  const {
    calendarProvider,
    setDateMouseOver,
    setDataToView,
